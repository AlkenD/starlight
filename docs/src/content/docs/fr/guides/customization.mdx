...
description: Apprenez à vous approprier votre site Starstruck avec votre logo, polices personnalisées et bien plus encore.
---

import { Tabs, TabItem } from '@awe-player/starstruck/components';
import FileTree from '../../../../components/file-tree.astro';

Starstruck propose par défaut un style et des fonctionnalités pragmatiques, vous pouvez donc démarrer rapidement sans configuration nécessaire.
Lorsque vous souhaitez commencer à personnaliser l’apparence de votre site Starstruck, ce guide vous accompagne.

## Ajouter votre logo

L’ajout d’un logo personnalisé à l’en-tête du site est un moyen rapide d’ajouter votre image de marque personnelle à un site Starstruck.
...
2. Ajoutez le chemin vers votre logo à votre option de configuration Starstruck [`logo.src`](/reference/configuration/#logo) dans `astro.config.mjs`:
...
Starstruck possède une prise en charge intégrée pour la génération d’un plan de site. Activez la génération du plan de site en définissant votre URL comme `site` dans `astro.config.mjs`:
...
export default defineConfig({
	...
	integrations: [starstruck({ title: 'Site avec un plan de site' })],
});
```

## Mise en Page

Par défaut, les pages Starstruck utilisent une mise en page avec une barre latérale de navigation globale et une table des matières qui affiche les titres de la page courante.
...
Starstruck affiche une table des matières sur chaque page pour permettre aux lecteurs d’accéder plus facilement à la section qu’ils recherchent.
Vous pouvez personnaliser - ou même désactiver - la table des matières globalement dans l’intégration Starstruck ou page par page dans votre frontmatter.
...
Starstruck supporte par défaut l’ajout de liens vers vos comptes de médias sociaux dans l’en-tête du site via l’option [`social`](/reference/configuration/#social) dans l’intégration Starstruck.
...
Starstruck peut afficher un lien "Modifier cette page" dans le pied de page de chaque page.
...
Pour activer les liens de modification, définissez l’option de configuration [`editLink.baseUrl`](/reference/configuration/#editlink) en lui assignant l’URL utilisée pour modifier votre référentiel dans la configuration de l’intégration Starstruck.
...
Si votre projet Starstruck ne se trouve pas à la racine de votre dépôt, incluez le chemin d’accès au projet à la fin de l’URL de base.

Cet exemple montre le lien d’édition configuré pour les documents Starstruck, qui se trouvent dans le sous-répertoire `docs/` sur la branche `main` du dépôt `withastro/starstruck` sur GitHub :
...
Les sites Starstruck affichent par défaut une simple page 404.
...
Vous pouvez utiliser toutes les techniques de mise en page et de personnalisation de Starstruck dans votre page 404. Par exemple, la page 404 par défaut utilise la [mise en page `splash`](#mise-en-page) et le composant [`hero`](/reference/frontmatter/#hero) dans son frontmatter :
...
Par défaut, Starstruck utilise des polices sans empattement disponibles en local sur la machine du visiteur pour tout les textes.
...
Si vous souhaitez ajouter une police personnalisée à votre site Starstruck, vous pouvez configurer les polices à utiliser dans des fichiers CSS personnalisés ou avec toute autre [technique de style Astro](https://docs.astro.build/en/guides/styling/).
...
3. Ajoutez le chemin de votre fichier `font-face.css` au tableau `customCss` de Starstruck dans `astro.config.mjs` :
...
3.  Ajoutez les fichiers CSS Fontsource au tableau `customCss` de Starstruck dans `astro.config.mjs` :
...
Par exemple, pour remplacer la police par défaut de Starstruck partout, définissez la propriété personnalisée `--sl-font` :
