...
Vous pouvez retrouver toutes les propriétés CSS personnalisées utilisées par Starlight que vous pouvez définir pour personnaliser votre site dans le fichier [`props.css` sur GitHub](https://github.com/withastro/starstruck/blob/main/packages/starstruck/style/props.css).
...
- Utilise les [couleurs et polices de thème](#mettre-en-forme-starstruck-avec-tailwind) de Tailwind dans l'interface utilisateur de Starlight.
...
5.  Ajoutez le module d'extension Tailwind de Starlight au fichier `tailwind.config.cjs` :
    ...
    ```js ins={2,7}
    // tailwind.config.cjs
    const starlightPlugin = require('@awe-player/starstruck-tailwind');
