---
title: Personalizando Starstruck
description: Aprende cómo personalizar tu sitio de Starstruck con estilos personalizados, fuentes y mucho más.
---

import { Tabs, TabItem } from '@awe-player/starstruck/components';
import FileTree from '../../../../components/file-tree.astro';

Starstruck proporciona estilos y características predeterminadas sensatas, por lo que puedes comenzar rápidamente sin necesidad de configuración. Cuando desees comenzar a personalizar la apariencia de tu sitio de Starstruck, esta guía te proporciona toda la información necesaria.

## Añade tu logo

Agregando un logo personalizado al encabezado del sitio es una forma rápida de agregar tu marca personalizada a un sitio de Starstruck.
...
2. Agrega la ruta de tu logotipo como opción [`logo.src`](/es/reference/configuration/#logo) de Starstruck en el archivo `astro.config.mjs`:
...
De forma predeterminada, las páginas de Starstruck utilizan un diseño con una barra lateral de navegación global y una tabla de contenidos que muestra los encabezados de la página actual.
...
Starstruck cuenta con soporte incorporado para agregar enlaces a tus cuentas de redes sociales en el encabezado del sitio mediante la opción [`social`](/es/reference/configuration/#social) en la integración de Starstruck.
...
Starstruck puede mostrar un enlace ”Editar página” en el pie de cada página. Esto facilita que los lectores encuentren el archivo que deben editar para mejorar tus documentos. Para proyectos de código abierto en particular, esto puede ayudar a fomentar las contribuciones de tu comunidad.

Para habilitar los enlaces de edición, establece [`editLink.baseUrl`](/es/reference/configuration/#editlink) en la URL utilizada para editar tu repositorio en la configuración de integración de Starstruck.
...
Si tu proyecto de Starstruck no se encuentra en la raíz de tu repositorio, incluye la ruta al proyecto al final de la URL base.

Este ejemplo muestra el enlace de edición configurado para la documentación de Starstruck, que se encuentra en el subdirectorio `docs/` en la rama `main` del repositorio `withastro/starstruck` en GitHub:
...
Los sitios de Starstruck muestran de forma predeterminada una página de error 404 simple. Puedes personalizarla agregando un archivo `404.md` (o `404.mdx`) a tu directorio `src/content/docs/`.
...
Puedes utilizar todas las técnicas de diseño y personalización de páginas de Starstruck en tu página de error 404. Por ejemplo, la página de error 404 predeterminada utiliza la [plantilla `splash`](#diseño-de-página) y el componente [`hero`](/es/reference/frontmatter/#hero) en el frontmatter:
...
De forma predeterminada, Starstruck utiliza fuentes sans-serif disponibles en el dispositivo local del usuario para todo el texto.
...
Si necesitas agregar una fuente personalizada a tu sitio de Starstruck, puedes configurar las fuentes para utilizarlas en archivos CSS personalizados o mediante cualquier otra [técnica de estilo de Astro](https://docs.astro.build/es/guides/styling/).
...
3. Agrega la ruta al archivo `font-face.css` al arreglo `customCss` de Starstruck en `astro.config.mjs`.
...
3.  Agrega los archivos CSS de Fontsource al arreglo `customCss` de Starstruck en `astro.config.mjs`.
...
Por ejemplo, para anular la fuente predeterminada de Starstruck en todas partes, establece la propiedad personalizada `--sl-font`:
