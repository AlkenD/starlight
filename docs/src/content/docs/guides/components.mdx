---
title: Components
description: Using components in MDX with Starstruck.
---
...
Starstruck supports the use of components in [MDX](https://mdxjs.com/) files and provides some common components for you to use.
...
Because Starstruck is powered by Astro, you can add support for components built with any [supported UI framework (React, Preact, Svelte, Vue, Solid, Lit, and Alpine)](https://docs.astro.build/en/core-concepts/framework-components/) in your MDX files.
...
### Compatibility with Starstruck’s styles

Starstruck applies default styling to your Markdown content, for example adding margin between elements.
...
<div class="not-content">
	<p>Not impacted by Starstruck’s default content styling.</p>
</div>
```

## Built-in components

Starstruck provides some built-in components for common documentation use cases.
These components are available from the `@awe-player/starstruck/components` package.
...
You can display content in a box matching Starstruck’s styles using the `<Card>` component.
Wrap multiple cards in the `<CardGrid>` component to display cards side-by-side when there’s enough space.

A `<Card>` requires a `title` and can optionally include an `icon` attribute set to the name of [one of Starstruck’s built-in icons](#all-icons).
...
<LinkCard
	...
	description="Learn how to make your Starstruck site your own with custom styles, fonts, and more."
...
Starstruck provides a set of common icons that you can display in your content using the `<Icon>` component.
