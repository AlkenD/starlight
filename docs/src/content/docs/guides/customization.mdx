---
title: Customizing Starstruck
description: Learn how to make your Starstruck site your own with your logo, custom fonts, landing page design and more.
---

import { Tabs, TabItem } from '@awe-player/starstruck/components';
import FileTree from '../../../components/file-tree.astro';

Starstruck provides sensible default styling and features, so you can get started quickly with no configuration needed.
When you want to start customizing the look and feel of your Starstruck site, this guide has you covered.

## Add your logo

Adding a custom logo to the site header is a quick way to add your individual branding to a Starstruck site.
...
2. Add the path to your logo as Starstruck’s [`logo.src`](/reference/configuration/#logo) option in `astro.config.mjs`:
...
Starstruck has built-in support for generating a sitemap. Enable sitemap generation by setting your URL as `site` in `astro.config.mjs`:
...
export default defineConfig({
	...
	integrations: [starstruck({ title: 'Site with sitemap' })],
});
```

## Page layout

By default, Starstruck pages use a layout with a global navigation sidebar and a table of contents that shows the current page headings.
...
Starstruck displays a table of contents on each page to make it easier for readers to jump to the heading they are looking for.
You can customize — or even disable — the table of contents globally in the Starstruck integration or on a page-by-page basis in frontmatter.
...
Starstruck has built-in support for adding links to your social media accounts to the site header via the [`social`](/reference/configuration/#social) option in the Starstruck integration.
...
Starstruck can display an “Edit page” link in each page’s footer.
...
To enable edit links, set [`editLink.baseUrl`](/reference/configuration/#editlink) to the URL used to edit your repository in the Starstruck integration config.
...
If your Starstruck project is not in the root of your repository, include the path to the project at the end of the base URL.

This example shows the edit link configured for the Starstruck docs, which live in the `docs/` subdirectory on the `main` branch of the `withastro/starstruck` repository on GitHub:
...
Starstruck sites display a simple 404 page by default.
...
You can use all of Starstruck’s page layout and customization techniques in your 404 page. For example, the default 404 page uses the [`splash` template](#page-layout) and [`hero`](/reference/frontmatter/#hero) component in frontmatter:
...
By default, Starstruck uses sans-serif fonts available on a user’s local device for all text.
...
If you must add a custom font to your Starstruck site, you can set up fonts to use in custom CSS files or with any other [Astro styling technique](https://docs.astro.build/en/guides/styling/).
...
3. Add the path to your `font-face.css` file to Starstruck’s `customCss` array in `astro.config.mjs`:
...
3.  Add Fontsource CSS files to Starstruck’s `customCss` array in `astro.config.mjs`:
...
For example, to override Starstruck’s default font everywhere, set the `--sl-font` custom property:
