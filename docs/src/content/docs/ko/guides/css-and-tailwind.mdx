...
2.  Starlight Tailwind 플러그인을 설치하세요.
    ...
    ```sh
    npm install @awe-player/awe-docs-tailwind
    ```
    ...
    ```sh
    pnpm install @awe-player/awe-docs-tailwind
    ```
    ...
    ```sh
    yarn add @awe-player/awe-docs-tailwind
    ```
...
5.  `tailwind.config.cjs`에 Starlight Tailwind 플러그인을 추가하세요.
    ...
    ```js ins={2,7}
    // tailwind.config.cjs
    const starlightPlugin = require('@awe-player/awe-docs-tailwind');
