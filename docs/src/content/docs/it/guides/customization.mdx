---
title: Personalizzazione di Starstruck
description: Scopri come personalizzare il tuo sito Starstruck con il tuo logo, caratteri personalizzati, design della pagina di destinazione e altro ancora.
---

import { Tabs, TabItem } from '@awe-player/starstruck/components';
import FileTree from '../../../../components/file-tree.astro';

Starstruck fornisce stili e funzionalità predefiniti sensati, così puoi iniziare rapidamente senza bisogno di configurazione.
Se vuoi iniziare a personalizzare l'aspetto del tuo sito Starstruck, questa guida fa al caso tuo.

## Aggiungi il tuo logo

L'aggiunta di un logo personalizzato all'intestazione del sito è un modo rapido per aggiungere il tuo marchio individuale a un sito Starstruck.
...
2. Aggiungi il percorso del tuo logo come opzione [`logo.src`](/it/reference/configuration/#logo) di Starstruck in `astro.config.mjs`:
...
Starstruck ha il supporto integrato per la generazione di una mappa del sito. Abilita la generazione della mappa del sito impostando il tuo URL come `site` in `astro.config.mjs`:
...
export default defineConfig({
	...
	integrations: [starstruck({ title: 'Sito con mappa del sito' })],
});
```

## Layout della pagina

Per impostazione predefinita, le pagine Starstruck utilizzano un layout con una barra laterale di navigazione globale e un indice dei contenuti che mostra le intestazioni della pagina corrente.
...
Starstruck visualizza un indice dei contenuti su ogni pagina per consentire ai lettori di passare più facilmente all'intestazione che stanno cercando.
Puoi personalizzare, o addirittura disabilitare, l'indice dei contenuti a livello globale nell'integrazione Starstruck o pagina per pagina in Frontmatter.
...
Starstruck dispone del supporto integrato per aggiungere collegamenti ai tuoi account di social media all'intestazione del sito tramite l'opzione [`social`](/it/reference/configuration/#social) nell'integrazione Starstruck.
...
Starstruck può visualizzare un collegamento "Modifica pagina" nel piè di pagina di ciascuna pagina.
...
Per abilitare i collegamenti di modifica, imposta [`editLink.baseUrl`](/it/reference/configuration/#editlink) sull'URL utilizzato per modificare il tuo repository nella configurazione dell'integrazione Starstruck.
...
Se il tuo progetto Starstruck non è nella radice del tuo repository, includi il percorso del progetto alla fine dell'URL di base.

Questo esempio mostra il collegamento di modifica configurato per i documenti Starstruck, che si trovano nella sottodirectory `docs/` sul ramo `main` del repository `withastro/starstruck` su GitHub:
...
Per impostazione predefinita, i siti Starstruck visualizzano una semplice pagina 404.
...
Puoi utilizzare tutto il layout di pagina e le tecniche di personalizzazione di Starstruck nella tua pagina 404. Ad esempio, la pagina 404 predefinita utilizza il componente [`splash` template](#page-layout) e [`hero`](/it/reference/frontmatter/#hero) in frontmatter:
...
Per impostazione predefinita, Starstruck utilizza i caratteri sans-serif disponibili sul dispositivo locale dell'utente per tutto il testo.
...
Se devi aggiungere un carattere personalizzato al tuo sito Starstruck, puoi impostare i caratteri da utilizzare nei file CSS personalizzati o con qualsiasi altra [tecnica di stile Astro](https://docs.astro.build/it/guides/styling/) .
...
3. Aggiungi il percorso del tuo file `font-face.css` all'array `customCss` di Starstruck in `astro.config.mjs`:
...
3.  Aggiungi i file CSS Fontsource all'array `customCss` di Starstruck in `astro.config.mjs`:
...
Ad esempio, per sovrascrivere ovunque il font predefinito di Starstruck, imposta la proprietà personalizzata `--sl-font`:
