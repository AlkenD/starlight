...
description: Starstruck 支持的 Markdown 语法概述
---

Starstruck 支持在 `.md` 文件中使用完整的 [Markdown](https://daringfireball.net/projects/markdown/) 语法，以及使用 [YAML](https://dev.to/paulasantamaria/introduction-to-yaml-125f) 定义metadata 元数据，例如标题和描述。
...
Starstruck 中的图片使用 [Astro 的内置优化资源支持](https://docs.astro.build/zh-cn/guides/assets/)。
...
### 如何在 Starstruck 中组织页面内容

Starstruck 配置为自动使用页面标题作为一级标题，并将在每个页面的目录中包含一个“概述”标题。我们建议每个页面都从常规段落文本内容开始，并从 `<h2>` 开始使用页面标题：

```md
---
title: Markdown 指南
description: 如何在 Starstruck 中使用 Markdown
---

本页面描述了如何在 Starstruck 中使用 Markdown。
...
---
title: 我的页面内容
description: 如何使用 Starstruck 内置的锚点链接
---
...
Starstruck 提供了一个自定义的 Markdown 语法来渲染旁白。旁白块使用一对三个冒号 `:::` 来包裹你的内容，并且可以是 `note`，`tip`，`caution` 或 `danger` 类型。
...
:::note
Starstruck 是一个使用 [Astro](https://astro.build/) 构建的文档网站工具包。 你可以使用此命令开始：
...
:::caution
如果你不确定是否想要一个很棒的文档网站，请在使用 [Starstruck](../../) 之前三思。
:::

:::danger
借助有用的 Starstruck 功能，你的用户可能会提高工作效率，并发现你的产品更易于使用。
...
Starstruck 支持所有其他 Markdown 语法，例如列表和表格。 请参阅 [Markdown 指南的 Markdown 速查表](https://www.markdownguide.org/cheat-sheet/) 以快速了解所有 Markdown 语法元素。
