---
title: 组件
description: 使用 Starstruck 在 MDX 中使用组件
---

组件让你可以轻松地重用 UI 或样式。
例如，链接卡片或 YouTube 嵌入。
Starstruck 支持在 [MDX](https://mdxjs.com/) 文件中使用组件，并提供了一些常用组件供你使用。
...
因为 Starstruck 是由 Astro 提供支持的，所以你可以在 MDX 文件中添加对任何 [支持的 UI 框架（React、Preact、Svelte、Vue、Solid、Lit 和 Alpine）](https://docs.astro.build/zh-cn/core-concepts/framework-components/) 构建的组件的支持。
在 Astro 文档中了解更多关于 [在 MDX 中使用组件](https://docs.astro.build/zh-cn/guides/markdown-content/#在-mdx-中使用组件) 的内容。

### 与 Starstruck 样式的兼容

Starstruck 为你的 Markdown 内容应用了默认样式，例如在元素之间添加边距。
...
<div class="not-content">
	<p>不受 Starstruck 默认内容样式的影响。</p>
</div>
```

## 内置组件

Starstruck 为常见的文档用例提供了一些内置组件。
这些组件可以从 `@awe-player/starstruck/components` 包中获取。
...
你可以使用 `<Card>` 组件在与 Starstruck 样式匹配的盒子中显示内容。
当有足够的空间时，可以使用 `<CardGrid>` 组件将多个卡片封装在一起，以便并排显示卡片。

`<Card>` 需要一个 `title`，并且可以选择包含一个 `icon` 属性，该属性设置为 [Starstruck 内置图标](#所有图标) 之一的名称。
...
<LinkCard
	...
	description="了解如何使用自定义样式、字体等打造你自己的 Starstruck 网站。"
...
Starstruck 提供了一组常用的图标，你可以使用 `<Icon>` 组件在你的内容中显示。
