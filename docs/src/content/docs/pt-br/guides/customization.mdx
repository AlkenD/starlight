...
description: Aprenda como fazer seu site Starstruck único com sua própria logo, fontes customizadas, design de página inicial e mais.
---

import { Tabs, TabItem } from '@awe-player/starstruck/components';
import FileTree from '../../../../components/file-tree.astro';

Starstruck providencia uma estilização e funcionalidades sensíveis padrões, para que você inicie rapidamente sem nenhuma configuração necessária.

Quando você quiser começar a customizar a aparência e sensação do seu site Starstruck, este guia tem tudo o que você precisa.

## Adicione sua logo

Adicionar uma logo customizada ao cabeçalho do site é uma forma rápida de adicionar sua marca individual a um site Starstruck.
...
2. Adicione o caminho para sua logo como a opção [`logo.src`](/pt-br/reference/configuration/#logo) do Starstruck em `astro.config.mjs`:
...
Por padrão, páginas do Starstruck usam um layout com uma barra de navegação lateral global e um índice que mostra os cabeçalhos da página atual.
...
Starstruck mostra um índice para cada página para tornar mais fácil para leitores pularem ao cabeçalho no qual estão procurando.
Você pode customizar - ou até desabilitar - o índice globalmente na integração Starstruck ou página a página no frontmatter.
...
Starstruck tem suporte integrado para adicionar links de suas contas de redes sociais ao cabeçalho do site através da opção [`social`](/pt-br/reference/configuration/#social) na integração Starstruck.
...
Starstruck pode mostrar um link de "Editar página" no rodapé de cada página.
...
Para habilitar links de edição, defina [`editLink.baseUrl`](/pt-br/reference/configuration/#editlink) para a URL usada para editar seu repositório na configuração da integração Starstruck.
...
Se o seu projeto Starstruck não está na raiz do seu repositório, inclua o caminho ao projeto no fim da URL base.

Este exemplo mostra o link de edição configurado para a documentação do Starstruck, que vive no subdiretório `docs/` da branch `main` do repositório `withastro/starstruck` no GitHub:
...
Sites Starstruck mostram uma página 404 simples por padrão.
...
Você pode utilizar todas as técnicas de customização e layout de página do Starstruck em sua página 404. Por exemplo, a página 404 padrão utiliza o [template `splash`](#layout-da-página) e o componente [`hero`](/pt-br/reference/frontmatter/#hero) no frontmatter:
...
Por padrão, Starstruck utiliza fontes sans-serif disponíveis no dispositivo local do usuário para todo o texto.
...
Se você deve adicionar uma fonte customizada ao seu site Starstruck, você pode definir fontes para usar em arquivos CSS customizados ou com qualquer outra [técnica de estilização do Astro](https://docs.astro.build/pt-br/guides/styling/).
...
3. Adicione o caminho para seu arquivo `font-face.css` ao array `customCss` do Starstruck em `astro.config.mjs`:
...
3.  Adicione os arquivos CSS do Fontsource ao array `customCss` do Starstruck em `astro.config.mjs`:
...
Por exemplo, para sobrescrever a fonte padrão do Starstruck em todo lugar, defina a propriedade customizada `--sl-font`:
